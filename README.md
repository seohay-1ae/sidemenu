...
```bash
git checkout -b feature/브랜치이름  # dev에서 새로운 작업 브랜치 생성
git push -u origin feature/브랜치이름  # 원격 저장소에도 브랜치 올리기
```
> 💡 브랜치 이름 예시: `feature/product_detail-youngshin`
...
git checkout feature/브랜치이름  # 다시 작업 브랜치로 이동
...
- 📝 Pull Request는 **feature → dev**로 생성해주세요.
